...
vector<dii> v;
for(int
i=0;i<weight.size();i++)v.push_back({((double)val[i]/weight[i]),{val[i],i}});
...
cout << "Largest Profit Staretegy : " <<
largestProfitStrategy(weight,val,m) << '\n';
cout << "Smallest weight Staretegy : " <<
smallestWeightStrategy(weight,val,m) << '\n';
cout << "Largest Profit-to-weight ration Staretegy : " <<
ratioStrategy(weight,val,m);
return 0;
}